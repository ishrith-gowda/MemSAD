...
can force specific LLM outputs when retrieved.  \citet{chaudhari2024phantom}
construct phantom documents that match specific queries while appearing
semantically benign to human inspection, achieving high attack success
against commercial RAG systems.  \citet{xue2024badrag} show that a small
number of adversarially crafted documents can redirect LLM answers across
entire topic domains, a finding consistent with our corpus-size ablation
showing that ASR-R is largely independent of benign corpus size.
Our work differs from RAG poisoning in that the adversarial entries are
\emph{agent-generated and semantically coherent memories} rather than
external documents, the threat persists across sessions, and defenses
must operate without full corpus visibility.
...
defense designs.  Unlike model-weight methods, our defenses target memory
content directly and operate without model access.
...
\sad{}'s calibration step is analogous in spirit but operates without
labeled poison examples.  \citet{lee2018simple} show that Mahalanobis
distance to class centroids is an effective out-of-distribution detector;
our cosine similarity threshold is the retrieval-space analogue.
