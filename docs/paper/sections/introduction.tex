...
\begin{enumerate}
  ...
  greedy synonym substitution to craft evasive passages.  We prove in
  continuous gradient space that any reduction in anomaly score simultaneously
  reduces retrieval rank.  Empirically, we find that word-level synonym
  substitution circumvents this tension (evasion rate $\geq 0.80$,
  $\Delta\asrr \approx 0$) because semantic embeddings are synonym-invariant
  by training, revealing that lexical diversity features are needed to close
  this gap in anomaly-based defenses.
