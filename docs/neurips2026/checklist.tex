...
\item {\bf Claims}
    ...
    \item[] Justification: All four contributions listed in Section~\ref{sec:intro} are supported by theoretical results (Theorems~\ref{thm:gradient_coupling}--\ref{thm:generalization}, Lemma~\ref{lem:certified}, Propositions~\ref{prop:hardness}--\ref{prop:memsad_plus}) and experimental validation in Section~\ref{sec:experiments}.

\item {\bf Limitations}
    \item[] Question: Does the paper discuss the limitations of the work performed by the authors?
    \item[] Answer: \answerYes{}
    \item[] Justification: Section~\ref{sec:discussion} includes a dedicated limitations paragraph covering (i)~corpus scale, (ii)~regret bound drift assumptions, (iii)~piecewise-linear encoder scope of the hardness result, and (iv)~the residual synonym loophole in \memsad{}+.
...
\item {\bf Broader impacts}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: Positive impact: the \memsad{} defense, certified detection radius, and composite portfolio directly improve the security of production memory-augmented agents. Negative impact: the attack reproductions (\agentpoison{}, \minja{}, \injecmem{}) could be misused, though each attack is already published in prior work and we do not introduce new attack capabilities. Every attack is paired with a working defense in Section~\ref{sec:experiments}, and the synonym loophole (Proposition~\ref{prop:synonym}) is disclosed transparently so the community can investigate mitigations rather than be surprised by it.
