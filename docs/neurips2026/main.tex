...
The combined score $s_{\text{comb}}$ also preserves directional coupling (Corollary~\ref{cor:combined_coupling}, Appendix~\ref{app:tradeoff_surface}).
...
Since $\rank(\enc(c)) \leq k$, we have $s(c; \cH) \geq \bar{s} + \Delta_s$. The gap condition and calibration bound together force $s(c; \cH) > \hat{\mu} + \kappa\hat{\sigma}$. Full proof in Appendix~\ref{app:proofs}.
\end{proof}
...
\paragraph{\memsad{}+.} We extend \memsad{} with character $n$-gram JSD features that are provably not synonym-invariant (Proposition~\ref{prop:memsad_plus}, Appendix~\ref{app:memsad_plus}). Empirically, \memsad{}+ raises \injecmem{} TPR from $0.00$ (synonym-evaded) to $0.40$ and original from $0.20$ to $0.60$.
...
Measured $\asrt = 0.20$ under GPT-4o-mini is $3.4\times$ lower than the modelled $0.68$~\citep{chen2024agentpoison}, reflecting production safety alignment (Figures~\ref{fig:measured_asr_a},~\ref{fig:matrix}, Appendix~\ref{app:additional_figures}).
...
\paragraph{Standalone \memsad{}.}
At $\kappa=2.0$, standalone \memsad{} achieves moderate TPR at strict FPR control; practitioners should treat it as one layer of a composite defense, not a standalone solution.

\paragraph{Limitations.}
(i)~Corpus scale: $|\cM| = 1{,}000$; large-scale evaluation remains future work, though cross-corpus NQ results (Appendix~\ref{app:nq_eval}) provide partial validation.
