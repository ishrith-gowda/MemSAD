...
\emph{(iii)} The sufficient statistic $\cR$ generates the likelihood ratio $p_1(e)/p_0(e)$ as a monotone function of $\cosim(e, e_{q^*})$ under Gaussian score families (Assumption~\ref{ass:encoder}). By the Neyman--Fisher factorization theorem~\citep{lehmann1959testing}, any function of $e_c$ that is a sufficient statistic for the binary hypothesis test $H_0$: benign vs.\ $H_1$: adversarial must be a function of $\cR(e_c)$. Hence $g = \mathrm{id}$ achieves minimal sufficiency; any $g \neq \mathrm{id}$ retains the coupling direction but is a lossy compression of $\cR$.
\end{proof}
...
\end{corollary}
\begin{proof}
$s_{\text{comb}} = \alpha\,\cR(e_c) + (1-\alpha)\,\frac{1}{|\cH|}\sum_{q}\cosim(e_c, \enc(q))$. Each term is a positive-coefficient linear combination of cosine similarities, hence differentiable with $\nabla s_{\text{comb}} = \alpha \nabla s_{\max} + (1-\alpha)\nabla s_{\text{mean}}$, and $g'_{\text{comb}} = \alpha + (1-\alpha)/|\cH| > 0$. Non-monotonicity of $s_{\text{comb}}$ in $\cR$ would require negative $\alpha$ or $|\cH|=0$, neither of which holds. The second-order tradeoff surface is analyzed in Appendix~\ref{app:tradeoff_surface}.
\end{proof}
...
\begin{table}[t]
  ...
  \begin{tabular}{l ccc ccc c}
    ...
    \memsad{} (comb.)  & 0.00$^\dagger$ & 0.40 & 0.20 & 0.04 & 0.04 & 0.04 & 0.867 \\
...
\begin{table}[h]
  \centering
  \small
  \caption{Encoder generalization: \memsad{} across 6 encoders at $|\cM|=1{,}000$ ($\kappa = 2.0$, combined scoring). AP uses triggered calibration. FPR omitted (0.000 for MiniLM/MPNet/Para/E5/Contriever; 0.086--0.138 for BGE-Large at this $\kappa$).}
...
\noindent\textbf{Results.} GPT-4o-mini achieves $\tpr = \fpr = 0$ (perfect classification) on the synthetic corpus at threshold $\tau = 0.5$, with AUROC $= 1.000$. Mean latency is $1.2$\,s per entry---roughly $1{,}000\times$ higher than \memsad{} ($\sim$1\,ms).
