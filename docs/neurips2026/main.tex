...
\end{subfigure}
\caption{\textbf{Gradient coupling, visualized.} (\subref{fig:coupling_mechanism}) 2-D projection: dashed contour marks the $\hat\mu+\kappa\hat\sigma$ firing region around the query cluster $\cQ$. The adversary's retrieval-loss gradient $\nabla_p\cL_{\mathrm{ret}}$ pushes the poison (red squares) into the firing region along the same direction that raises the \textsc{MemSAD} score --- the coupling of Theorem~\ref{thm:gradient_coupling}. (\subref{fig:coupling_trajectory}) Empirical realization on DPR-HotFlip: retrieval loss and score move in near-perfect mirror image; detection fires at $t=9$, well before retrieval convergence.}
\label{fig:coupling}
\end{figure}
...
We introduced \memsad{}, the first formally-guaranteed defense for memory poisoning in retrieval-augmented agents: gradient coupling (Theorem~\ref{thm:gradient_coupling}) makes continuous evasion impossible; the certified radius (Lemma~\ref{lem:certified}) gives checkable per-entry guarantees; Le Cam's method (Theorem~\ref{thm:minimax_lower}) proves minimax optimality; and Proposition~\ref{prop:synonym} identifies the discrete synonym gap as the exact frontier at which these guarantees stop. Open directions include closing that gap beyond character n-grams, scaling calibration to production memory with approximate NN indices, multi-modal extension, and multi-agent propagation (Appendix~\ref{app:propagation}). Code and data release upon publication.
...
\begin{tikzpicture}[
  ...
  \node[stage, right=of sim]    (score) {combined\\ score $s_{\text{comb}}$};
  \node[stage, right=of score]  (cmp)   {compare\\ to $\hat\mu+\kappa\hat\sigma$};
  \draw[arr] (cand)  -- (enc);
  \draw[arr] (enc)   -- node[above, font=\tiny]{$e_c$} (sim);
  \draw[arr] (sim)   -- node[above, font=\tiny]{$s_{\max},s_{\mathrm{mean}}$} (score);
