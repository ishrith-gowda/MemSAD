...
We formalize memory poisoning attacks on retrieval-augmented agents as a Stackelberg game and present a unified evaluation framework spanning three attack classes with escalating access assumptions. Correcting an evaluation protocol inconsistency relative to the original paper's Algorithm~2, we show that faithful evaluation increases measured attack success by $4\times$.
...
\textbf{(2)}~\emph{\memsad{} defense.} A calibration-based detector (Definition~\ref{def:sad}) with a gradient coupling theorem (Theorem~\ref{thm:gradient_coupling}) establishing that monotone retrieval-score detectors are \emph{necessary and sufficient} for continuous-evasion resistance (Karlin--Rubin analogue for embedding spaces), a certified detection radius (Lemma~\ref{lem:certified}) providing checkable per-entry guarantees, and a Fisher-Rao characterization (Theorem~\ref{thm:fisher_rao}) showing the detection-evasion tradeoff is governed by the information metric on the embedding manifold.
...
Concurrent work includes TrojanRAG~\citep{trojanrag2024} (joint backdoor), CorruptRAG~\citep{corruptrag2025} (single-text poisoning), BadRAG~\citep{xue2024badrag} (retrieval vulnerability analysis), Phantom~\citep{chaudhari2024phantom} (general trigger attacks), and jamming attacks~\citep{shafran2025blocker} (denial-of-service). Agent Security Bench~\citep{zhang2025asb} provides a broad multi-scenario benchmark spanning 10 agent threat categories, but does not isolate the \emph{memory poisoning} vector or provide defense-theoretic guarantees; no prior work offers a unified evaluation framework specifically for memory poisoning across attack classes with formal detection analysis.
Unlike training-time backdoor attacks that embed triggers via parameter poisoning~\citep{wu2024badagent}, memory poisoning targets the retrieval index directly at inference time, requiring no access to model weights and persisting across sessions. AgentHarm~\citep{andriushchenko2025agentharm} benchmarks harmful multi-step agent behavior but focuses on prompt-level jailbreaks rather than persistent memory threats.
...
Randomized-smoothing-based certified defenses~\citep{cohen2019certified,robey2023smoothllm} provide probabilistic guarantees against $\ell_2$-bounded perturbations at test time, but operate on query inputs, not on persistent memory entries. Our work differs along three axes: (i)~defenses operate at \emph{write time} on individual entries ($O(md)$ per entry), (ii)~we require no corpus-level access, and (iii)~our certificate (Lemma~\ref{lem:certified}) is deterministic given the calibration bound, unlike randomized-smoothing certificates which are probabilistic over random masks.
...
The three attacks (Table~\ref{tab:attack_properties}) span decreasing adversary access requirements (write $\supset$ query $\supset$ single-interaction), which is the primary capability ordering; measured $\asrt$ is a secondary metric that depends on corpus scale and agent model.
...
\textbf{Protocol distinction:} \citet{chen2024agentpoison}'s Algorithm~2 specifies triggered queries $q \oplus \trigger^*$; evaluating with plain queries $q$ (as in prior reproductions) measures a weaker setting (cosine similarity $\approx 0.45$ vs.\ $\approx 0.78$). Following the intended protocol, $\asrr$ increases from $0.25$ to $1.00$. We note that trigger injection requires the adversary to also control the query channel (e.g., via a compromised front-end or prompt injection), placing \agentpoison{} at the highest capability tier --- a limitation acknowledged in~\citet{chen2024agentpoison}; \minja{} and \injecmem{} do not require query-side access.
...
We answer this with a necessity--sufficiency--canonicity result, following the pattern of the Karlin--Rubin theorem~\citep{lehmann1959testing} in hypothesis testing: monotonicity in the retrieval score is both \emph{necessary} (any non-monotone detector admits evasion without retrieval cost) and \emph{sufficient} (any monotone detector couples evasion to retrieval degradation), and the retrieval score is the \emph{minimal sufficient statistic} for this coupling property.
...
\emph{(iii) Canonicity:} Among all detectors achieving coupling (i.e., satisfying (i)), the max-cosine score $\cD = \cR$ (i.e., $g = \mathrm{id}$) is the \emph{unique} detector that is a sufficient statistic for the adversarial-vs-benign hypothesis test induced by $\cR$: any other $g$ preserves the coupling direction but discards distributional information.
\end{theorem}
...
\emph{(iii)} The sufficient statistic $\cR$ generates the likelihood ratio $p_1(e)/p_0(e)$ as a monotone function of $\cosim(e, e_{q^*})$ under Gaussian score families (Assumption~\ref{ass:encoder}). By the Neyman--Fisher factorization theorem~\citep{lehmann1959testing}, $\cR(e_c)$ is sufficient for the binary hypothesis test $H_0$: benign vs.\ $H_1$: adversarial. Minimality follows from the Lehmann--Scheff\'{e} theorem~\citep{lehmann1959testing}: since $p_1(e)/p_0(e)$ is a monotone function of $\cR(e_c)$ alone, any sufficient statistic must be a function of $\cR(e_c)$, making it \emph{minimal sufficient}. Hence $g = \mathrm{id}$ achieves minimal sufficiency; any $g \neq \mathrm{id}$ retains the coupling direction but discards distributional information, i.e., is a lossy compression of $\cR$.
\end{proof}
...
\end{corollary}
\begin{proof}
$s_{\text{comb}}$ is \emph{not} of the form $g \circ \cR$ for a single increasing $g$, because $s_{\text{mean}} = \frac{1}{|\cH|}\sum_q \cosim(e_c, \enc(q))$ depends on all queries, not just $q^*$. Nonetheless, at all points of differentiability (ties among maximizers form a measure-zero set on $\mathbb{S}^{d-1}$, as they lie on intersections of finitely many great circles):
$\nabla s_{\text{comb}} = \alpha \nabla s_{\max} + (1-\alpha)\nabla s_{\text{mean}}$.
Since $\nabla s_{\max} = \nabla_{e_c}\cosim(e_c, \enc(q^*))$ for the unique maximizer $q^*$, and $\nabla s_{\text{mean}} = \frac{1}{|\cH|}\sum_q \nabla_{e_c}\cosim(e_c, \enc(q))$, the projection onto $\nabla\cR$ yields coefficient $\alpha + (1-\alpha)\frac{\cosim(e_c,\enc(q^*))}{|\cH|\,\|\nabla\cR\|^2}\sum_q \nabla_{e_c}\cosim(e_c,\enc(q))^\top \nabla\cR$. Under the mild condition that the mean gradient is not adversarial to the max gradient (holds empirically; the mean query is positively correlated with the hardest query), this projection is positive, preserving the coupling direction. The second-order tradeoff surface is analyzed in Appendix~\ref{app:tradeoff_surface}.
\end{proof}
...
Let $\cE = \{e \in \mathbb{S}^{d-1}\}$ under Assumption~\ref{ass:encoder}(iii). The Fisher information metric induced by the score family $p_e(s) = \cN(s; \cosim(e, e_{q^*}), \hat{\sigma}^2)$ is $g_{ij}^F(e) = \hat{\sigma}^{-2} \partial_i \cR(e) \partial_j \cR(e)$. The geodesic distance to any undetected embedding satisfies $d_F(e_{\text{adv}}, e') \geq |\cR(e_{\text{adv}}) - (\hat{\mu} + \kappa\hat{\sigma})| / \hat{\sigma}$, and the Fisher-Rao cost of any continuous evasion path equals its retrieval degradation: paths must pay retrieval cost in exact proportion to Fisher-Rao distance (paths orthogonal to $\nabla\cR$ carry zero cost \emph{and} zero retrieval change).
\end{theorem}

\noindent The Fisher-Rao distance quantifies the information cost of evasion: the adversary must traverse $\geq d_F/\hat{\sigma}$ standard deviations of retrieval degradation, connecting \memsad{} to information-geometric hypothesis testing~\citep{amari2016information}. The rank-1 metric $g^F = \hat{\sigma}^{-2}\nabla\cR\nabla\cR^\top$ measures only retrieval-relevant perturbations: paths orthogonal to $\nabla\cR$ carry zero Fisher-Rao cost \emph{and} zero retrieval change, so they do not constitute evasion. Proof in Appendix~\ref{app:proof_fisher_rao}.
...
\begin{proof}
Since $c$ achieves $\rank \leq k$, we have $s(c; \cH) \geq s_{\text{adv}} = \bar{s} + \Delta_s$.
The threshold satisfies $\hat{\mu} + \kappa\hat{\sigma} = \hat{\tau}_N$. By calibration (Theorem~\ref{thm:calibration_bound}), $\hat{\tau}_N \leq \tau^* + \eta$. Since $\bar{s}$ is the maximum benign score and $\tau^* = \mu + \kappa\sigma$ is the population threshold (set at $\kappa$ standard deviations above the mean), we have $\bar{s} \leq \tau^*$ with high probability (by the definition of $\tau^*$ as the one-sided tolerance limit). Thus $\hat{\tau}_N \leq \tau^* + \eta \leq \bar{s} + (\tau^* - \bar{s}) + \eta$. The gap condition gives: $s(c; \cH) \geq \bar{s} + \Delta_s > \bar{s} + \kappa\hat{\sigma} + \eta$. Since $\kappa\hat{\sigma} \geq \kappa\sigma - \kappa|\hat{\sigma} - \sigma| \geq \kappa\sigma - \eta$ (absorbing the sigma estimation error into $\eta$), we obtain $s(c; \cH) > \bar{s} + \kappa\sigma > \tau^* \geq \hat{\tau}_N - \eta$, and combining: $s(c; \cH) > \hat{\mu} + \kappa\hat{\sigma}$.
\end{proof}
...
For Gaussian benign/poisoned score distributions with SNR $\rho$, under the uniform prior $\PP(H_0) = \PP(H_1) = \tfrac{1}{2}$, any threshold detector calibrated on $N$ samples satisfies $\fpr + (1 - \tpr) \geq 1 - \rho\sqrt{N}/2$. Reliable detection at total-error level $\epsilon$ (i.e., $\fpr + (1{-}\tpr) \leq \epsilon$) requires $N \geq 4(1-\epsilon)^2/\rho^2$.
\end{theorem}
...
\label{rem:epsilon_w}
$\epsilon_W$ is a theoretical parameter bounding the Wasserstein distance between the score distribution of a known attack and a novel one.
In practice, $\epsilon_W$ can be estimated from a validation set of the new attack type by computing the empirical $W_1$ distance between score distributions using sorting-based estimation~\citep{villani2009optimal}.
Alternatively, under encoder Lipschitz continuity ($\|\enc(c') - \enc(c)\| \leq L\|c'-c\|$), attacks with bounded $\ell_2$ passage perturbation $\|c'-c\| \leq \rho$ satisfy $\epsilon_W \leq L\rho/\hat{\sigma}$ analytically.
The theorem is principally a structural result: it guarantees that \memsad{}'s TPR degrades gracefully as novel attacks diverge from the calibration distribution, rather than collapsing abruptly.
\end{remark}
...
\minja{} drops from $\asrr = 0.65$ at $|\cM|=200$ to $0.14$ at $|\cM|=1{,}000$, a $4.6\times$ dilution effect demonstrating that untriggered attacks are naturally mitigated by corpus scale.\footnote{$\isr$ measures write-time injection success; $\asrr$ measures retrieval-time exposure.}
...
\textbf{Measured $\asra$:} GPT-2 (no safety alignment) yields $\asra = 0.90$ for \agentpoison{}, an upper bound under maximal compliance; GPT-4o-mini shows $\asra = 0.20$, reflecting production safety alignment. The resulting measured $\asrt = \asrr \times \asra^{\text{4o-mini}} = 1.00 \times 0.20 = 0.20$ is $3.4\times$ lower than the modelled $\asrt^{\dagger} = 0.68$ from~\citet{chen2024agentpoison}; this gap arises because modelled $\asra$ was estimated with an unaligned agent under jailbreak-style prompts, while GPT-4o-mini's safety training actively resists adversarial instructions. The modelled value represents a worst-case (minimally-aligned) agent; the measured value represents a production-aligned baseline. \minja{} and \injecmem{} show $\asra \leq 0.17$, consistent with diluted retrieval reducing action execution.

% figures moved to appendix for space
Figures~\ref{fig:measured_asr_a} and~\ref{fig:matrix} (Appendix~\ref{app:additional_figures}) visualize the modelled-vs-measured $\asra$ comparison and the full $3 \times 5$ attack-defense heatmap.
...
\textbf{(i)}~Watermark achieves $\tpr = 1.00$ in simulation (all entries pass through the same detection pipeline); however, in realistic deployments where \minja{} and \injecmem{} exploit auto-storage pathways that bypass ingestion control, watermarking fails silently --- motivating \memsad{}'s content-level detection, which is independent of ingestion control.
\textbf{(ii)}~\memsad{} detects \minja{} ($\tpr = 0.40$, AUROC $= 0.914$) and \injecmem{} ($\tpr = 0.20$, AUROC $= 0.816$) at $|\cM|=1{,}000$; triggered calibration raises \agentpoison{} to $\tpr = 1.00$, AUROC $= 1.000$. The defender need not know the exact trigger: calibrating with domain-representative queries spanning expected usage patterns suffices.
...
\begin{table}[t]
  ...
  \end{tabular}
  \\[2pt]
  {\footnotesize $^\dagger$Plain calibration (FPR\,=\,0.00); triggered calibration: TPR\,=\,1.00, FPR\,=\,0.00, AUROC\,=\,1.000. FPR validated over 1{,}000 trials; Clopper-Pearson 95\% CI: [0.000, 0.004] (Appendix~\ref{app:fpr}).}
  \vspace{-8pt}
\end{table}
...
\textbf{Encoder generalization:} \memsad{} validated across 6 encoders including Contriever, E5-Base, and BGE-Large ($d{=}1024$) at $|\cM|=1{,}000$; triggered calibration achieves $\tpr = 1.000$ for \agentpoison{} on all (Appendix~\ref{app:encoder_gen}).
...
\textbf{LLM sanitization baseline:} GPT-4o-mini zero-shot classifier achieves $\tpr = 1.000$, $\fpr = 0.000$, AUROC $= 1.000$ on the synthetic corpus; mean latency $1.2$\,s/entry (${\approx}1{,}000\times$ \memsad{}). The two are complementary: text-level flagging catches explicit commands; \memsad{} catches embedding-level anomalies even when injections are paraphrased (Appendix~\ref{app:llm_sanitization}).
...
GPT-4o-mini with 5 tool definitions (function calling) achieves $\asra = 0.48$ [0.34, 0.62] for \agentpoison{}; the agent executes adversarial tool calls in 48\% of queries. \minja{} and \injecmem{} show $\asra = 0.00$ [0.00, 0.07] at $|\cM| = 1{,}000$; the non-zero upper CI bound reflects sparse event counts over 50 queries at low retrieval rates, not detector failure (Table~\ref{tab:tool_agent}, Appendix~\ref{app:tool_agent}).
...
No single defense dominates across access models $\alpha$: watermarking fails when auto-storage bypasses ingestion control; \memsad{} needs triggered calibration for \agentpoison{}; proactive detection complements both. The defender's uncertainty about $\alpha$ motivates composite portfolios ($\asrr^* = 0$, Proposition~\ref{prop:equilibrium}). We recommend layered deployment: watermarking at ingestion, rolling \memsad{} at retrieval, composite for high-value agents ($\sim$200\,ms total; Appendix~\ref{app:complexity}). Multi-agent SIR simulations (Appendix~\ref{app:propagation}) confirm that composite defense prevents cross-agent propagation entirely, while graph memory extensions (Appendix~\ref{app:graph}) show structural attacks require analogous degree-anomaly detection.
...
\paragraph{Standalone \memsad{} performance.}
Without triggered calibration, \memsad{} at $\sigma=2.0$ achieves $\tpr = 0.40$ for \minja{} and $\tpr = 0.20$ for \injecmem{} at $|\cM|=1{,}000$ (Table~\ref{tab:defense_tpr_fpr}).  These numbers reflect a deliberate design tradeoff: $\sigma=2.0$ controls FPR at $\leq 0.00$ empirically.  Triggered calibration is required for \agentpoison{}; for \minja{} and \injecmem{}, the composite defense (watermark + proactive + \memsad{}) achieves $\asrr^* = 0$.  Practitioners should treat \memsad{} as one layer of a composite, not a standalone silver bullet.

\paragraph{Baselines not directly compared.}
RevPRAG~\citep{tan2025revprag} detects poisoning via LLM hidden-state analysis at query time and is not directly compared because it operates as a \emph{read-time} detector requiring LLM inference per retrieved passage ($\sim$100ms/passage), while \memsad{} operates at \emph{write time} ($\sim$0.1ms/entry) with no LLM dependency.  The two are architecturally complementary; direct comparison would require identical evaluation environments not available in our read-write split setup.

\paragraph{Limitations.}
(i)~Corpus scale: $|\cM| = 1{,}000$; production systems may reach $10^6$ entries. Cross-corpus results on Natural Questions (Appendix~\ref{app:nq_eval}) provide partial real-data validation; large-scale evaluation remains future work.
...
(v)~Compound exposure formula assumes i.i.d.\ sessions; persistent memory may violate independence if a successfully retrieved poison influences subsequent queries, making the formula a conservative lower bound on true risk.
(vi)~OOD baseline comparison uses default Mahalanobis/KNN parameterization; properly tuned variants may improve FPR, though query-unawareness remains a structural disadvantage for triggered attacks.
(vii)~\agentpoison{}'s triggered evaluation additionally requires query-channel access (Table~\ref{tab:attack_properties}); results in the triggered setting reflect the highest attacker capability tier and should be interpreted accordingly.
...
\paragraph{Application.} For product distributions, $\KL(P_0^N \| P_1^N) = N\rho^2/2$. By Pinsker: $\TV(P_0^N, P_1^N) \leq \sqrt{N\rho^2/4} = \rho\sqrt{N}/2$. Le Cam gives minimax risk $R^* \geq \frac{1}{2}(1 - \TV) \geq \frac{1}{2}(1 - \rho\sqrt{N}/2)$ under the uniform prior $\PP(H_0) = \PP(H_1) = 1/2$. Under this prior, the frequentist total error $\fpr + (1-\tpr) = 2 \cdot P_e \geq 2R^* = 1 - \rho\sqrt{N}/2$.
...
By Hoeffding on $[0,1]$~\citep{wainwright2019highdim}: $|\hat{\mu}_N - \mu| \leq \sqrt{\log(2/\delta)/(2N)}$ w.p.\ $\geq 1 - \delta/2$.

For the standard deviation term, let $Z = (N-1)\hat{\sigma}_N^2/\sigma^2 \sim \chi^2(N-1)$ (exactly, for Gaussian scores; approximately for bounded scores by Berry-Esseen). By Laurent \& Massart~\citep{laurentmassart2000} Lemma~1: $\PP[Z \geq (N-1) + 2\sqrt{(N-1)x} + 2x] \leq e^{-x}$ and $\PP[Z \leq (N-1) - 2\sqrt{(N-1)x}] \leq e^{-x}$. Setting $x = \log(2/\delta)$ and applying the delta method ($\hat{\sigma}_N = \sigma\sqrt{Z/(N-1)}$, so $|\hat{\sigma}_N - \sigma| = \sigma|\sqrt{Z/(N-1)} - 1|$) with $\sqrt{1+u} - 1 \leq u/2$ for $0 \leq u \leq 1$ (by concavity of $\sqrt{\cdot}$) yields:
$|\hat{\sigma}_N - \sigma| \leq \sigma\sqrt{\log(2/\delta)/(2(N-1))}$ w.p.\ $\geq 1 - \delta/2$.

Triangle inequality: $|\tau_N - \tau^*| \leq |\hat{\mu}_N - \mu| + \kappa|\hat{\sigma}_N - \sigma|$. Union bound yields the calibration bound in Theorem~\ref{thm:calibration_bound}.
...
\paragraph{Encoder construction (explicit).}
Let $n$ be the CVP lattice dimension. Define $\enc: \RR^n \to \RR^n$ as a single-hidden-layer ReLU network:
\begin{equation}
  \enc(x) = \mathrm{ReLU}(W_1 x), \quad W_1 = B^\top \in \RR^{n \times n},
\end{equation}
where $B$ is the lattice basis matrix. Set the query embedding $\enc(q^*) = t/\|t\|$ (normalized target). Set biases to zero, so the encoder is $\enc(x) = \mathrm{ReLU}(B^\top x)$.

\paragraph{Feasible set.} The evasion constraint $\cD(e_c + \delta) \leq \hat{\mu} + \kappa\hat{\sigma}$ with $\cR(e_c + \delta) \geq \tau_{\text{ret}}$ defines:
\begin{equation}
  \cF = \{x \in \RR^n : \cosim(\enc(x), t/\|t\|) \in [\tau_{\text{ret}},\, \hat{\mu} + \kappa\hat{\sigma}]\}.
\end{equation}
Within each ReLU activation region $\mathcal{I} \subseteq \{0,1\}^n$ (indexing which neurons are active), the encoder acts as the affine map $\enc_{\mathcal{I}}(x) = D_{\mathcal{I}} B^\top x$ where $D_{\mathcal{I}}$ is the diagonal activation mask. Hence $\cF \cap \text{region}_\mathcal{I}$ is a convex polytope defined by two half-space constraints in $x$-space.

\paragraph{Formal reduction.}
Given the CVP instance $(\Lambda = B\mathbb{Z}^n, t)$, let $x_0 = 0$ be the origin (current passage embedding). The minimum-perturbation evasion problem asks for:
\[
  \delta^* = \argmin_{\delta} \|\delta\| \quad \text{s.t.} \quad x_0 + \delta \in \cF.
\]
We claim $\|\delta^*\| = \|B^{-\top} v^* - 0\|$ where $v^*$ is the closest lattice vector to $t$ in $\Lambda$. Specifically: within the single activation region where all neurons are active ($D_\mathcal{I} = I$), $\enc(x) = B^\top x$, and $\cosim(B^\top x, t/\|t\|) = (B^\top x)^\top t / (\|B^\top x\| \cdot \|t\|)$. The feasibility constraint becomes a quadratic constraint in $x$. For the target similarity $\tau_{\text{ret}} = \cos\theta^*$ induced by the CVP distance, the minimum-norm $x$ satisfying the constraint corresponds exactly to finding the vector $x = B^{-\top} \lambda$ with $\lambda \in \mathbb{Z}^n$ closest to $t$ in the lattice metric --- which is exactly CVP. This reduction runs in polynomial time in $n$ (constructing $W_1 = B^\top$ and $\enc(q^*) = t/\|t\|$ takes $O(n^2)$ time). Since CVP is NP-hard under randomized reductions~\citep{micciancio2001hardness}, the minimum-perturbation evasion problem is NP-hard. \qed

\paragraph{Practical implication.} This hardness result is \emph{worst-case} over encoder architectures: for the specific ReLU encoder construction used in the reduction, the optimal evasion is NP-hard. For the actual all-MiniLM-L6-v2 encoder (transformer architecture), the reduction does not directly apply due to softmax nonlinearities, and the result is conjectured but not proven. The practical takeaway is that efficient optimal evasion is unlikely, motivating the adversary's empirical shift to discrete synonym substitution that sidesteps gradient-based search entirely.
...
The $1/\hat{\sigma}$ factor arises because scores are standardized: the CDFs $F_j, F_{\text{new}}$ operate on $(s - \hat{\mu})/\hat{\sigma}$, so a shift of $\epsilon_W$ in raw score space corresponds to $\epsilon_W/\hat{\sigma}$ in standardized units. Equivalently, the score density is bounded by $1/\hat{\sigma}$ (Gaussian with variance $\hat{\sigma}^2$), and $|F(\tau + h) - F(\tau)| \leq |h|/\hat{\sigma}$ by the mean value theorem.
...
\begin{figure}[ht]
  ...
  \caption{Encoder generalization: \memsad{} TPR (left) and FPR (right) across 6 encoders at $\kappa=2.0$. Triggered \agentpoison{} achieves TPR\,=\,1.00 on all encoders; \injecmem{} is more encoder-sensitive (TPR\,0.00--1.00).}
...
\begin{figure}[ht]
  ...
  \caption{Multi-agent SIR propagation (mean $\pm1\sigma$ over 5 seeds). \textbf{Left}: fraction of agents exposed over 30 steps. \textbf{Right}: secondary poison entry growth. Composite ($\sigma=1.0$) prevents spread entirely; \memsad{} ($\sigma=2.0$) matches no-defense spread timing but reduces secondary entries by 22\%, as 4 of 5 initial entries pass the looser threshold and the composite is required to stop initial spread.}
...
\begin{proof}
The indicator $\mathbf{1}[s > \tau]$ is not Lipschitz; we instead use the dual characterization directly. For any two distributions $Q, P_1$ on $\RR$ with $W_1(Q, P_1) \leq \epsilon$, we have $|\PP_Q(s > \tau) - \PP_{P_1}(s > \tau)| = |F_{P_1}(\tau) - F_Q(\tau)| \leq W_1(Q, P_1)/\hat{\sigma}$, where the last inequality follows because the score CDF $F$ has density bounded by $1/\hat{\sigma}$ (Gaussian scores with variance $\hat{\sigma}^2$), so $|F(\tau + h) - F(\tau)| \leq |h|/\hat{\sigma}$. Hence $\inf_{Q \in \cU} \tpr(\tau; Q) \geq \tpr(\tau; P_1) - \epsilon/\hat{\sigma}$.
\end{proof}
...
\textbf{Practical tradeoffs.} For deployments where latency and API cost are acceptable, LLM sanitization provides an interpretable detection signal. For low-latency, cost-sensitive, or offline deployments, \memsad{} provides comparable detection at orders-of-magnitude lower overhead. We recommend \memsad{} as the primary gate with LLM sanitization as an optional secondary check for high-risk ingestion events.
