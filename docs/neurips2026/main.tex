...
\textbf{Encoder generalization:} \memsad{} validated across 6 encoders including OpenAI embeddings; triggered calibration achieves $\tpr = 1.000$ for \agentpoison{} on all (Appendix~\ref{app:encoder_gen}).
...
(iv)~We use a single primary encoder (MiniLM-L6); cross-encoder transfer is validated on 6 encoders including Contriever and OpenAI (Appendix~\ref{app:encoder_gen}).
