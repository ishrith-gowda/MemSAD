...
Randomized-smoothing-based certified defenses for LLMs~\citep{robey2023smoothllm} provide probabilistic guarantees against prompt-level perturbations but operate on query inputs, not on persistent memory entries. Our work differs along three axes: (i)~defenses operate at \emph{write time} on individual entries ($O(md)$ per entry), (ii)~we require no corpus-level access, and (iii)~our certificate (Lemma~\ref{lem:certified}) is deterministic given the calibration bound, unlike randomized-smoothing certificates which are probabilistic over random masks.
