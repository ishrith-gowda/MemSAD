...
Figure~\ref{fig:threat_model} illustrates the interaction. The three attacks (Table~\ref{tab:attack_properties}) span decreasing adversary access requirements (write $\supset$ query $\supset$ single-interaction), which is the primary capability ordering; measured $\asrt$ is a secondary metric that depends on corpus scale and agent model.
...
\noindent The full pre-ingestion procedure is given in Algorithm~\ref{alg:sad}.
...
At $N = 200$, $\delta = 0.05$: the excess error is $\leq 0.096$, meaning \memsad{}'s empirical FPR of $0.000$ implies true FPR $\leq 0.096$ with 95\% confidence. The Clopper-Pearson validation (Table~\ref{tab:fpr_validation}) provides a tighter bound of $0.004$ by exploiting the binomial structure.
