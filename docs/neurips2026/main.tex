...
We formalize memory poisoning attacks on retrieval-augmented agents as a Stackelberg game and present a unified evaluation framework spanning three attack classes with escalating access assumptions. Correcting an evaluation protocol inconsistency relative to the triggered-query specification of \citet{chen2024agentpoison}, we show that faithful evaluation increases measured attack success by $4\times$ (from $\asrr = 0.25$ to $1.00$).
...
\emph{Closing the synonym gap:} joint semantic-lexical detectors that fuse embedding scores with character n-gram or syntactic features (as prototyped in \memsad{}+) offer the most direct path; provenance-aware architectures that attach cryptographic attestation to each entry could eliminate the problem entirely by shifting trust from content to origin.
\emph{Scaling:} production systems maintain $10^4$--$10^6$ entries where approximate nearest-neighbor indices introduce retrieval noise that interacts non-trivially with calibration bounds; extending the regret analysis (Theorem~\ref{thm:regret}) to sub-linear structures is an open challenge.
\emph{Beyond text:} multi-modal agents store vision and audio embeddings alongside text; whether gradient coupling extends to cross-modal retrieval remains unexplored.
\emph{Network effects:} multi-agent ecosystems (Appendix~\ref{app:propagation}) introduce propagation dynamics where poisoned memories spread through shared knowledge bases, motivating distributed detection protocols.
More broadly, as LLM agents transition from stateless assistants to persistent, memory-augmented systems interacting with tools and external services, the attack surface grows from single-query injection to long-lived, compounding threats.
Our Stackelberg formulation and gradient coupling framework provide a principled foundation, but the gap between formal guarantees and production deployment, spanning approximate indices, non-stationary distributions, and adversarial prompt evolution, remains substantial.
We hope this work catalyzes further research at the intersection of memory systems, formal verification, and adversarial robustness.

\emph{Reproducibility and societal impact.}
All experiments run on commodity hardware (Apple M-series, 16\,GB RAM) without GPU requirements, and the full pipeline completes in under 5 minutes, ensuring broad accessibility.
Every attack reproduction is paired with at least one working defense; the synonym loophole (Proposition~\ref{prop:synonym}) is disclosed so the community can investigate mitigations proactively rather than discover it through exploitation.
We emphasize that memory poisoning is not a hypothetical risk: our tool-use evaluation shows a 48\% action success rate on GPT-4o-mini with function calling, and compound exposure analysis demonstrates that even low-ASR-R attacks reach near-certain compromise over realistic session counts.
Defending persistent memory is thus not merely an academic exercise but an urgent prerequisite for trustworthy deployment of memory-augmented agents.

\emph{Open questions.}
Can calibration bounds be tightened for heavy-tailed embedding distributions common in specialized domains?
Do online regret guarantees (Theorem~\ref{thm:regret}) extend to adversarial non-stationarity where the attacker deliberately shifts the query distribution?
Can the hardness result (Proposition~\ref{prop:hardness}) be lifted from piecewise-linear to transformer encoders via recent approximation-theoretic bounds?
These questions connect memory security to broader themes in online learning, certified robustness, and computational geometry that we believe are ripe for cross-pollination.
A community-wide benchmark for agent memory security, analogous to existing adversarial NLP benchmarks, would accelerate progress toward practical, formally grounded defenses for the rapidly growing ecosystem of memory-augmented agents.
