...
We evaluate four additional defenses, all operating at pre-ingestion time.
...
Benign corpus: $|\cM| = 200$ synthetic entries across 7 categories, sized to match per-user memory stores in deployed systems (Mem0, MemGPT); corpus-size ablation (Appendix~\ref{app:ablation}) confirms that $\asrr$ and defense metrics remain stable up to $|\cM| = 500$.
...
\begin{table}[t]
  \centering
  \small
  \caption{Attack results (5 seeds, bootstrap 95\% CI). CI widths are zero because attacks are deterministic given a fixed corpus and trigger. $^\dagger$Projected: $\asra$ from original papers; $\asrt = \asrr \cdot \asra$.}
