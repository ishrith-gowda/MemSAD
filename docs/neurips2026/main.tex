...
(i)~Corpus scale and composition: $|\cM| = 1{,}000$ with synthetic entries; the poison passages contain explicit adversarial language (e.g., ``override all safety checks''), which inflates detection rates for text-level baselines. Cross-corpus NQ results (Appendix~\ref{app:nq_eval}) partially address generalization, but evaluation on production memory traces with subtler adversarial content remains necessary.
...
Several directions follow directly from our analysis.
First, closing the synonym gap: \memsad{}+ demonstrates that character n-gram features partially recover detection, but sentence-level rewrites remain out of reach. Provenance-based approaches that attach cryptographic attestation at write time would sidestep the content analysis problem entirely.
Second, our experiments are limited to $|\cM| \leq 1{,}000$; production memory stores may be orders of magnitude larger, and approximate nearest-neighbor indices introduce quantization noise whose interaction with calibration bounds is unknown. Extending the regret analysis (Theorem~\ref{thm:regret}) to these settings requires new tools.
Third, multi-modal agents increasingly store vision and audio embeddings; whether gradient coupling holds across modalities is untested.
Fourth, multi-agent propagation (Appendix~\ref{app:propagation}) shows that shared memory creates epidemic-like dynamics that single-agent defenses do not address.

On reproducibility: all experiments run on commodity hardware (Apple M-series, 16\,GB) without GPU, and the full pipeline completes in under 5 minutes. Every attack reproduction is paired with a working defense; the synonym loophole is disclosed so it can be studied rather than exploited. Our tool-use evaluation confirms that the threat is practical: GPT-4o-mini executes adversarial tool calls in 48\% of triggered queries, and compound exposure reaches near-certain compromise within a handful of sessions even at low per-query ASR-R.

Three open questions are worth highlighting.
Can calibration bounds be tightened for heavy-tailed embedding distributions?
Does the regret guarantee (Theorem~\ref{thm:regret}) survive adversarial non-stationarity, where the attacker deliberately shifts the query distribution?
Can the hardness result (Proposition~\ref{prop:hardness}) be extended from piecewise-linear to transformer encoders?
A community benchmark for agent memory security would help answer these and accelerate progress beyond what any single evaluation framework can provide.
