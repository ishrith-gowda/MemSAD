...
(ii)~Measured $\asra$ (Table~\ref{tab:attack_results}) shows significant variation across agent models: GPT-2 lower bound ($0.90$) vs.\ GPT-4o-mini ($0.20$) for \agentpoison{}, reflecting the gap between proxy and production-scale evaluation.
...
(iv)~We use a single primary encoder (MiniLM-L6); cross-encoder transfer is validated on 5 encoders including Contriever and E5 at $|\cM|=1{,}000$ (Appendix~\ref{app:encoder_gen}).
