...
Memory poisoning compounds over sessions: $P(\text{compromise in } N \text{ sessions}) = 1 - (1 - \asrr)^{Nq}$ for $q$ queries/session.\footnote{This assumes i.i.d.\ Bernoulli trials across queries, which is a lower bound on true risk: persistent memory means a successfully retrieved adversarial entry may influence subsequent queries, further compounding exposure.} Even \minja{} at $\asrr = 0.14$ reaches 90\% compromise in 4 sessions; composite defense ($\asrr^* = 0$) is necessary (Table~\ref{tab:compound_exposure}, Appendix~\ref{app:compound}).
...
We instantiate the SIR model with $N=20$ agents, $p_{\text{re-store}}=0.30$, $T=30$ steps, 5 initial MINJA-style poison entries, and a 200-entry synthetic corpus (all-MiniLM-L6-v2 FAISS retrieval).  Three conditions are evaluated: (i)~no defense; (ii)~\memsad{} with triggered calibration ($\sigma=2.0$, combined scoring); (iii)~composite defense approximated by $\sigma=1.0$.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.92\linewidth]{fig_sir_propagation.pdf}
  \caption{Multi-agent SIR propagation. \textbf{Left}: fraction of agents exposed over 30 steps. \textbf{Right}: secondary poison entry growth. Composite ($\sigma=1.0$) prevents spread entirely; \memsad{} ($\sigma=2.0$) delays saturation but 4 of 5 initial entries pass the looser threshold.}
...
\begin{table}[h]
  \centering
  \small
  \caption{Multi-agent SIR propagation results ($N=20$, $p_{\text{re-store}}=0.30$, $T=30$, 5 initial poison entries).}
...
\noindent The tighter composite threshold quarantines 2 of 5 initial entries and all secondary entries, holding spread to zero. \memsad{} at $\sigma=2.0$ quarantines 5 total entries (1 initial, 4 secondary) but 4 initial entries already seed infection; spread reaches saturation 13 steps later than the no-defense baseline ($t_{90\%}$: 17\,vs.\,11 steps after secondary quarantine delays propagation).
