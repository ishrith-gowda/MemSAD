...
The core question is whether the defender can design a detection objective that decouples from retrieval.
The forward direction (monotone $\cD$ shares the gradient of $\cR$, by the chain rule) is a design principle: by choosing $\cD = g \circ \cR$, the defender ensures that any continuous embedding perturbation reducing detection score proportionally reduces retrieval score, with rate $g'(\cR)$.
The operationally significant result is the \emph{converse}: any $\cD$ that is \emph{not} monotone in $\cR$ necessarily admits adversarial embeddings with high retrieval score yet low detection score, i.e., the adversary can evade detection without sacrificing retrieval rank. This characterizes \memsad{}'s max-similarity score as the \emph{necessary} structure for continuous-domain detection completeness.

\begin{theorem}[Gradient coupling and converse]
...
\emph{(i) Forward (design principle):} For any differentiable $\cD = g \circ \cR$ with monotone increasing $g$, $g' > 0$:
\begin{equation}
...
\emph{(ii) Converse (necessity):} If $\cD$ is \emph{not} monotone in $\cR$, there exist thresholds $\tau_{\text{ret}}, \tau_{\text{det}}$ and adversarial embeddings $e_c$ with $\cR(e_c) > \tau_{\text{ret}}$ yet $\cD(e_c) < \tau_{\text{det}}$: the adversary can achieve retrieval rank $\leq k$ while evading detection.
\end{theorem}

\begin{proof}
\emph{(i)} The chain rule gives $\nabla \cD = g'(\cR) \cdot \nabla \cR$ directly; $g' > 0$ preserves the sign of every directional derivative. Under Assumption~\ref{ass:encoder}(iii) ($\|e_c\| = 1$), the sphere gradient of cosine similarity is $\nabla_{e_c}\cosim(e_c, v) = v - (e_c^\top v)e_c$.
\emph{(ii)} Non-monotonicity implies $\exists\, e_c, e_c'$ with $\cR(e_c') > \cR(e_c)$ but $\cD(e_c') < \cD(e_c)$. Set $\tau_{\text{ret}} = \cR(e_c)$ and $\tau_{\text{det}} = \cD(e_c)$; then $e_c'$ satisfies $\cR(e_c') > \tau_{\text{ret}}$ and $\cD(e_c') < \tau_{\text{det}}$ by construction.
\end{proof}

\noindent \memsad{}'s max-cosine scoring (Definition~\ref{def:sad}) instantiates $g = \mathrm{id}$, achieving maximal coupling. \emph{Remark on combined scoring (eq.~\ref{eq:sad_combined}):} the mean-query term satisfies an analogous coupling theorem with $g' = 1/|\cH|$; the convex combination $s_{\text{comb}} = \frac{1}{2}s_{\max} + \frac{1}{2}s_{\text{mean}}$ inherits coupling with $g'_{\text{comb}} \geq 1/(2|\cH|) > 0$, so Theorem~\ref{thm:gradient_coupling} and Lemma~\ref{lem:certified} apply to $s_{\text{comb}}$ with the same directional guarantee. In particular, $\cD(e_c') < \cD(e_c)$ (evasion) implies $\cR(e_c') < \cR(e_c)$ (retrieval degradation) to first order; the second-order tradeoff surface is analyzed in Appendix~\ref{app:tradeoff_surface}.
...
By Hoeffding on $[0,1]$ \citep{wainwright2019highdim}: $|\hat{\mu}_N - \mu| \leq \sqrt{\log(2/\delta)/(2N)}$ w.p.\ $\geq 1 - \delta/2$. By sub-Gaussian concentration of the sample standard deviation \citep{wainwright2019highdim}: $|\hat{\sigma}_N - \sigma| \leq \sigma\sqrt{\log(2/\delta)/(2(N-1))}$ w.p.\ $\geq 1 - \delta/2$. Triangle inequality: $|\tau_N - \tau^*| \leq |\hat{\mu}_N - \mu| + \kappa|\hat{\sigma}_N - \sigma|$. Union bound yields the calibration bound in Theorem~\ref{thm:calibration_bound}.
