...
\begin{table}[t]
  ...
  \end{tabular}
  \\[2pt]
  {\footnotesize $^\dagger$Plain calibration (FPR\,=\,0.00); triggered calibration: TPR\,=\,1.00, FPR\,=\,0.00, AUROC\,=\,1.000. FPR validated over 20 independent trials ($n = 1{,}000$ entries each); Clopper-Pearson 95\% CI: [0.000, 0.004] (Appendix~\ref{app:fpr}). \emph{Cross-table TPR consistency:} \minja{} TPR appears as $0.40$ here (10-passage poison set, plain calibration), $0.80$ in Table~\ref{tab:ood_comparison} (5-passage poison, MiniLM, triggered calibration), and varies $0.80$--$1.00$ across encoders in Table~\ref{tab:encoder_gen} (5-passage poison, triggered calibration). The differences are driven by poison-set size and calibration regime, not detector instability; each table's caption fixes the relevant conditions.}
  \vspace{-8pt}
\end{table}
...
No single defense dominates across access models $\alpha$: watermarking fails when auto-storage bypasses ingestion control; \memsad{} needs triggered calibration for \agentpoison{}; proactive detection complements both. The defender's uncertainty about $\alpha$ motivates composite portfolios ($\asrr^* = 0$, Proposition~\ref{prop:equilibrium}). We recommend layered deployment: watermarking at ingestion, rolling \memsad{} at write-time (i.e., applied to every candidate $c$ before commit; \emph{not} a retrieval-time filter), composite for high-value agents (${\sim}$2\,ms per entry; Table~\ref{tab:complexity}, Appendix~\ref{app:complexity}). Multi-agent SIR simulations (Appendix~\ref{app:propagation}) confirm that composite defense prevents cross-agent propagation entirely, while graph memory extensions (Appendix~\ref{app:graph}) show structural attacks require analogous degree-anomaly detection.
...
\paragraph{Comparison with concurrent post-retrieval defenses.}
...
\]
i.e., an entry is rejected if any of the watermark, MEMSAD, or proactive defenses flags it. The four-way breakdown (watermark, validation, proactive, MEMSAD) used as input to ablation tables refers to this same disjunction with one term ablated at a time. The weighted-sum form $0.50 d_{\text{wm}}(c) + 0.20 d_{\text{val}}(c) + 0.30 d_{\text{pro}}(c) > 0.50$ that appeared in some earlier exposition was an alternative tunable formulation we evaluated but is \emph{not} the composite reported in Tables~\ref{tab:defense_tpr_fpr} and \ref{tab:attack_defense_matrix}; we deprecate it here. Under the disjunctive composite, the perfect $\tpr = 1.00$ against \minja{} reported in Table~\ref{tab:defense_tpr_fpr} arises from MEMSAD detection (since \minja{} bypasses watermarking via auto-storage), consistent with the per-defense breakdown. The disjunction was chosen over the weighted sum because (i) it satisfies the assumption used in the equilibrium proof (Proposition~\ref{prop:equilibrium}) and (ii) its FPR is the union of per-defense FPRs, which are all $\leq 0.01$ at the calibrated thresholds and remain $\leq 0.04$ in aggregate.
