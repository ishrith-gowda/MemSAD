...
The combined score $s_{\text{comb}} = \alpha\, s_{\max} + (1-\alpha)\, s_{\text{mean}}$ with $\alpha \in (0,1)$ satisfies the gradient coupling (i) with $g'_{\text{comb}} = \alpha + (1-\alpha)/|\cH| > 0$, and admits the same necessity characterization (ii). Consequently, Lemma~\ref{lem:certified} and Theorem~\ref{thm:fisher_rao} apply unchanged to $s_{\text{comb}}$.
\end{corollary}
\begin{proof}
$s_{\text{comb}} = \alpha\,\cR(e_c) + (1-\alpha)\,\frac{1}{|\cH|}\sum_{q}\cosim(e_c, \enc(q))$. The mean term $s_{\text{mean}}$ is differentiable everywhere on $\mathbb{S}^{d-1}$. The max term $s_{\max} = \max_{q \in \cH}\cosim(e_c, \enc(q))$ is not differentiable when two queries achieve equal similarity (a tie); however, ties form a measure-zero set on $\mathbb{S}^{d-1}$ (intersection of finitely many great circles), so $s_{\max}$ is differentiable almost everywhere, and at each such point $\nabla s_{\max} = \nabla_{e_c}\cosim(e_c, \enc(q^*))$ for the unique maximizer $q^*$. At all points of differentiability: $\nabla s_{\text{comb}} = \alpha \nabla s_{\max} + (1-\alpha)\nabla s_{\text{mean}}$, with $g'_{\text{comb}} = \alpha + (1-\alpha)/|\cH| > 0$. Non-monotonicity of $s_{\text{comb}}$ in $\cR$ would require negative $\alpha$ or $|\cH|=0$, neither of which holds. The second-order tradeoff surface is analyzed in Appendix~\ref{app:tradeoff_surface}.
\end{proof}
